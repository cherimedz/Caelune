...
/* ── History page cards ── */
.stExpander {
  background: rgba(8,45,54,0.28) !important;
  border: 1px solid rgba(134,221,228,0.12) !important;
  border-radius: 12px !important;
...
def generate_pdf(content: str, title: str,
                 subject: str = "", fmt_label: str = "") -> bytes:
    ...
    pdf.set_fill_color(8, 45, 54)
    pdf.rect(0, 0, 210, 17, "F")
    pdf.set_font("Helvetica", "B", 10)
    pdf.set_text_color(249, 204, 203)
    pdf.set_xy(0, 4)
    pdf.cell(210, 8, "CAELUNE STUDY ASSISTANT", align="C")

    pdf.set_xy(20, 22)
    sub = " · ".join(filter(None, [subject, fmt_label]))
    if sub:
        pdf.set_font("Helvetica", "", 9)
        pdf.set_text_color(120, 120, 120)
        pdf.cell(0, 5, _safe_text(sub), new_x="LMARGIN", new_y="NEXT")

    pdf.set_draw_color(196, 53, 86)
    ...
    for line in content.split("\n"):
        ...
        if stripped.startswith("## "):
            ...
            pdf.set_text_color(8, 45, 54)
            pdf.multi_cell(0, 6, _safe_text(stripped[3:]))
            pdf.set_draw_color(134, 221, 228)
            ...
            pdf.multi_cell(0, 5, _safe_text(stripped[4:]))
        elif stripped.startswith(("- ", "* ")):
            pdf.set_font("Helvetica", "", 10)
            pdf.set_text_color(50, 50, 50)
            pdf.set_x(26)
            clean = _re.sub(r"\*\*([^*]+)\*\*", r"\1", stripped[2:])
            pdf.multi_cell(164, 5, _safe_text("•  " + clean))
            pdf.set_x(20)
        elif stripped.startswith("Q: "):
            pdf.ln(2)
            pdf.set_font("Helvetica", "B", 10)
            pdf.set_text_color(8, 45, 54)
            clean = _re.sub(r"\*\*([^*]+)\*\*", r"\1", stripped[3:])
            pdf.multi_cell(0, 5, _safe_text("Q: " + clean))
        elif stripped.startswith("A: "):
            pdf.set_font("Helvetica", "", 10)
            pdf.set_text_color(60, 60, 60)
            clean = _re.sub(r"\*\*([^*]+)\*\*", r"\1", stripped[3:])
            pdf.multi_cell(0, 5, _safe_text("A: " + clean))
            pdf.ln(2)
        else:
    ...
    pdf.cell(0, 5,
             _safe_text(f"Generated by Caelune · {datetime.now().strftime('%B %d, %Y')}"),
             align="C")
    return pdf.output()


def generate_docx(content: str, title: str,
                  subject: str = "", fmt_label: str = "") -> bytes:
    from docx import Document
    ...
    hdr = doc.add_heading("Caelune Study Assistant", 0)
    hdr.alignment = WD_ALIGN_PARAGRAPH.CENTER
    for run in hdr.runs:
        run.font.color.rgb = RGBColor(8, 45, 54)
    ...
    for line in content.split("\n"):
        ...
        if stripped.startswith("## "):
            ...
            r = p.add_run("Q: ")
            r.bold = True
            r.font.color.rgb = RGBColor(8, 45, 54)
            _add_md_runs(p, stripped[3:])
        elif stripped.startswith("A: "):
            p = doc.add_paragraph()
            r = p.add_run("A: ")
            r.bold = True
    ...
    fr = fp.add_run(f"Generated by Caelune · {datetime.now().strftime('%B %d, %Y')}")
...
""", unsafe_allow_html=True)
    ...
    if st.session_state.page == "history":
        ...
        if st.button("← Back to Notes", type="primary", key="back_from_history"):
        ...
        if st.session_state.history:
            ...
            if st.button(f"View History ({cnt})", key="view_history_btn", type="secondary"):
...
<div style="text-align:center;padding:0.5rem 0 1.6rem;">
  ...
  <p style="font-family:'DM Sans',sans-serif;font-size:0.93rem;color:rgba(134,221,228,0.55);font-weight:400;margin-top:5px;">
    Summaries generated this session
  </p>
</div>
""", unsafe_allow_html=True)
    ...
    if not _history:
        st.markdown(
            ...
            '<span>No summaries yet — go back and generate one</span></div>',
            unsafe_allow_html=True,
        )
    else:
        ...
        with _tc1:
            st.markdown(
                f'<p style="color:rgba(134,221,228,0.6);font-size:0.85rem;margin-bottom:0.6rem;">'
                f'{len(_history)} summar{"y" if len(_history)==1 else "ies"} this session</p>',
                unsafe_allow_html=True,
            )
        with _tc2:
            if st.button("Clear All", key="hist_clear_all", type="secondary"):
                st.session_state.history = []
                st.session_state.page = "main"
                st.rerun()

        for _hi, _entry in enumerate(reversed(_history)):
            _ridx = len(_history) - 1 - _hi
            _fl = FORMAT_LABELS.get(_entry.get("fmt", ""), "")
            _meta = " · ".join(filter(None, [
                _fl, _entry.get("subject", ""), _entry.get("difficulty", ""),
                _entry.get("language", ""), _entry.get("timestamp", ""),
            ]))
            with st.expander(
                f"{'◆' if _hi == 0 else '◇'}  {_entry['title']}",
                expanded=(_hi == 0),
            ):
                st.markdown(
                    f'<div style="font-size:0.78rem;color:rgba(134,221,228,0.6);'
                    f'margin-bottom:0.8rem;letter-spacing:0.3px;">{_meta}</div>',
                    unsafe_allow_html=True,
                )
                _rendered = _md.markdown(_entry["summary"], extensions=["nl2br", "tables"])
                st.markdown(
                    f'<div class="output-card md-output" style="max-height:280px;overflow-y:auto;">'
                    f'{_rendered}</div>',
                    unsafe_allow_html=True,
                )
                ...
                with _ac1:
                    if st.button("Restore →", key=f"hist_restore_{_ridx}", type="primary"):
                        st.session_state.last_summary = _entry["summary"]
                        st.session_state.last_terms = _entry.get("terms", "")
                        st.session_state.page = "main"
                        st.rerun()
                with _ac2:
                    _hfmt = st.radio(
                        "fmt", ["PDF", "Word (.docx)", "Plain text"],
                        horizontal=True, key=f"hist_dl_fmt_{_ridx}",
                        label_visibility="collapsed",
                    )
                with _ac3:
                    _es = _entry.get("subject", "")
                    _ef = FORMAT_LABELS.get(_entry.get("fmt", ""), "")
                    if _hfmt == "PDF":
                        _hd = generate_pdf(_entry["summary"], _entry["title"],
                                           subject=_es, fmt_label=_ef)
                        st.download_button("⬇ Download", data=_hd,
                                           file_name=f"caelune_{_ridx+1}.pdf",
                                           mime="application/pdf",
                                           key=f"hist_dl_{_ridx}")
                    elif _hfmt == "Word (.docx)":
                        _hd = generate_docx(_entry["summary"], _entry["title"],
                                            subject=_es, fmt_label=_ef)
                        st.download_button("⬇ Download", data=_hd,
                                           file_name=f"caelune_{_ridx+1}.docx",
                                           mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                                           key=f"hist_dl_{_ridx}")
                    else:
                        st.download_button("⬇ Download", data=_entry["summary"],
                                           file_name=f"caelune_{_ridx+1}.txt",
                                           mime="text/plain",
                                           key=f"hist_dl_{_ridx}")
    st.stop()
...
with col_right:
    ...
    with tab_summary:
        ...
        if st.session_state.last_summary:
            _sfmt = st.radio(
                "dl_fmt_s", ["PDF", "Word (.docx)", "Plain text"],
                horizontal=True, key="dl_fmt_summary", label_visibility="collapsed",
            )
            if _sfmt == "PDF":
                _sd = generate_pdf(
                    st.session_state.last_summary, "Summary",
                    subject=subject, fmt_label=FORMAT_LABELS.get(st.session_state.fmt, ""),
                )
                st.download_button("⬇ Download Summary", data=_sd,
                                   file_name="caelune_summary.pdf", mime="application/pdf",
                                   key="dl_summary_persistent")
            elif _sfmt == "Word (.docx)":
                _sd = generate_docx(
                    st.session_state.last_summary, "Summary",
                    subject=subject, fmt_label=FORMAT_LABELS.get(st.session_state.fmt, ""),
                )
                st.download_button("⬇ Download Summary", data=_sd,
                                   file_name="caelune_summary.docx",
                                   mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                                   key="dl_summary_persistent")
            else:
                st.download_button("⬇ Download Summary",
                                   data=st.session_state.last_summary,
                                   file_name="caelune_summary.txt", mime="text/plain",
                                   key="dl_summary_persistent")
    with tab_terms:
        terms_ph = st.empty()
        ...
        if st.session_state.last_terms:
            _tfmt = st.radio(
                "dl_fmt_t", ["PDF", "Word (.docx)", "Plain text"],
                horizontal=True, key="dl_fmt_terms", label_visibility="collapsed",
            )
            if _tfmt == "PDF":
                _td = generate_pdf(
                    st.session_state.last_terms, "Key Terms",
                    subject=subject, fmt_label=FORMAT_LABELS.get(st.session_state.fmt, ""),
                )
                st.download_button("⬇ Download Key Terms", data=_td,
                                   file_name="caelune_key_terms.pdf", mime="application/pdf",
                                   key="dl_terms_persistent")
            elif _tfmt == "Word (.docx)":
                _td = generate_docx(
                    st.session_state.last_terms, "Key Terms",
                    subject=subject, fmt_label=FORMAT_LABELS.get(st.session_state.fmt, ""),
                )
                st.download_button("⬇ Download Key Terms", data=_td,
                                   file_name="caelune_key_terms.docx",
                                   mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                                   key="dl_terms_persistent")
            else:
                st.download_button("⬇ Download Key Terms",
                                   data=st.session_state.last_terms,
                                   file_name="caelune_key_terms.txt", mime="text/plain",
                                   key="dl_terms_persistent")
